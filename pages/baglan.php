...
function imgUrl(?string $path, string $fallback = "../images/logo(2).webp"): string
{
  ...
  $resolved = resolveExistingImageWebPath($path);
  if ($resolved !== "") {
    return $resolved;
  }
  $resolvedFallback = resolveExistingImageWebPath($fallback);
  if ($resolvedFallback !== "") {
 ...
 *
 * @param array{name?:string,label?:string,hint?:string,mode?:string,url_value?:string,required?:bool} $opts
 */
function adminImageFieldHtml(string $assetBase, ?string $currentPath, array $opts = []): string
{
  $name = (string) ($opts["name"] ?? "resim");
  $label = (string) ($opts["label"] ?? "Görsel");
  $hint = (string) ($opts["hint"] ?? "");
  $mode = (string) ($opts["mode"] ?? "file");
  $required = !empty($opts["required"]);
  $urlValue = (string) ($opts["url_value"] ?? "");
  $current = trim((string) $currentPath);

  if ($mode === "url" && $urlValue === "" && $current !== "") {
    $urlValue = $current;
  }
  ...
  if ($mode === "url" && $urlValue !== "") {
    ...
    $previewSrc = adminImgUrl($assetBase, $current);
  }

  $hasPreview = $previewSrc !== "";
  $html = '<div class="admin-img-field">';
  $html .= '<div class="admin-img-preview">';
  $html .=
    ...
    " />";
  $html .=
    '<p class="admin-img-preview-empty" data-preview-empty' .
    ($hasPreview ? " hidden" : "") .
    ">Önizleme yok — görsel seçildiğinde burada görünür.</p>";
  $html .= "</div>";
  $html .=
  ...
  if ($mode === "url") {
    $html .=
      ...
      '" name="' .
      htmlspecialchars($name, ENT_QUOTES, "UTF-8") .
      '" data-preview-url placeholder="https://..." value="' .
      htmlspecialchars($urlValue, ENT_QUOTES, "UTF-8") .
      '"' .
      ($required ? " required" : "") .
      " />";
  } else {
    $html .=
      ...
      '" name="' .
      htmlspecialchars($name, ENT_QUOTES, "UTF-8") .
      '" accept="image/*" data-preview-input' .
      ($required ? " required" : "") .
      " />";
  }
...
function yardimciLinkLogoDefaults(): array
{
  return [
    ...
    "E-Belediye Sosyal Yardım Modülü" =>
      "../images/otomasyon/e-belediye-sosyal-yard-m-modulu_4432.webp",
    "Gebze Belediyesi" => "../images/yardimci_linkler/web_siteleri/gebze-belediyesi.webp",
    "Kocaeli Büyükşehir Belediyesi" =>
      "../images/yardimci_linkler/web_siteleri/kocaeli-buyuksehir-belediyesi.webp",
    ...
    "Türkiye Belediyeler Birliği" =>
      "../images/yardimci_linkler/bilgi_portallari/turkiye-belediyeler-birligi_2430.webp",
    "Cumhurbaşkanlığı Uzaktan Eğitim Kapısı" =>
      "../images/yardimci_linkler/bilgi_portallari/cumhur.webp",
...
function resolveExistingImageWebPath(string $webPath): string
{
  ...
  $normalized = normalizeImagePath($webPath);
  if (imageFileExistsExact($normalized)) {
    return $normalized;
  }

  $rel = preg_replace("#^\.\./#", "", $normalized);
  $dir = dirname($rel);
  ...
  foreach (imageAlternateExtensionsBaglan() as $ext) {
    $candidateRel =
      $dir === "." || $dir === "" ? $base . "." . $ext : $dir . "/" . $base . "." . $ext;
    $candidate = "../" . ltrim($candidateRel, "/");
    if (imageFileExistsExact($candidate)) {
      return $candidate;
    }
  }
...
function dbEnsureAnketKatilimlari(PDO $db): void
{
  ...
  try {
    ...
    if (dbTableExists($db, "anket_cevaplari")) {
      $db->exec(
...
{
  if ($anketId <= 0 || !dbTableExists($db, "anket_cevaplari")) {
    return false;
  }
  try {
    $row = dbFetchOne(
      $db,
      "SELECT id FROM anket_cevaplari WHERE anket_id = ? LIMIT 1",
      [$anketId],
    );
    return $row !== null;
  } catch (Throwable $e) {
...
/**
 * Anket sorularını seçenekleriyle yükler.
 *
 * @return list<array{id:int,soru_metni:string,soru_tipi:string,sira:int,secenekler:list<array{id:int,secenek_metni:string}>}>
 */
function adminFetchAnketSorulari(PDO $db, int $anketId): array
{
  if ($anketId <= 0 || !dbTableExists($db, "anket_sorulari")) {
    return [];
  }
  ...
  foreach ($rows as $row) {
    $soruId = (int) ($row["id"] ?? 0);
    $secenekler = [];
    if ($soruId > 0 && dbTableExists($db, "anket_secenekleri")) {
      $secenekler = dbFetchAll(
        $db,
    ...
    $out[] = [
      "id" => $soruId,
      "soru_metni" => (string) ($row["soru_metni"] ?? ""),
      "soru_tipi" => (string) ($row["soru_tipi"] ?? "coktan_secmeli"),
      "sira" => (int) ($row["sira"] ?? 0),
...
/**
 * POST'tan gelen soru/seçenek dizisini normalize eder.
 *
 * @return list<array{soru_metni:string,soru_tipi:string,sira:int,secenekler:list<string>}>
 */
function adminParseAnketSorularFromPost(array $post): array
{
  $raw = $post["sorular"] ?? [];
  if (!is_array($raw)) {
    return [];
  ...
  foreach ($raw as $item) {
    ...
    $metin = trim((string) ($item["metin"] ?? ""));
    if ($metin === "") {
      continue;
    }
    $tip = (string) ($item["tip"] ?? "coktan_secmeli");
    if ($tip !== "acik_uclu") {
      $tip = "coktan_secmeli";
    }
    $secenekler = [];
    if ($tip === "coktan_secmeli") {
      $opts = $item["secenekler"] ?? [];
      if (is_array($opts)) {
        foreach ($opts as $opt) {
...
/**
 * Anket soru + seçeneklerini kaydeder (öncekileri siler).
 * Cevabı olan ankette çağrılmamalıdır.
 */
function adminSaveAnketSorulari(PDO $db, int $anketId, array $sorular): void
{
  if ($anketId <= 0 || !dbTableExists($db, "anket_sorulari")) {
    return;
  }

  if (dbTableExists($db, "anket_secenekleri")) {
    $old = dbFetchAll($db, "SELECT id FROM anket_sorulari WHERE anket_id = ?", [
      $anketId,
    ]);
    foreach ($old as $oldRow) {
      $db
        ->prepare("DELETE FROM anket_secenekleri WHERE soru_id = ?")
        ->execute([(int) $oldRow["id"]]);
    }
  }

  $db->prepare("DELETE FROM anket_sorulari WHERE anket_id = ?")->execute([
    $anketId,
  ]);

  $insSoru = $db->prepare(
    "INSERT INTO anket_sorulari (anket_id, soru_metni, soru_tipi, sira) VALUES (?, ?, ?, ?)",
  );
  $insSec = dbTableExists($db, "anket_secenekleri")
    ? $db->prepare(
      "INSERT INTO anket_secenekleri (soru_id, secenek_metni) VALUES (?, ?)",
    )
    : null;
  ...
  foreach ($sorular as $soru) {
    ...
    $soruId = (int) $db->lastInsertId();
    if (
      $insSec &&
      $soru["soru_tipi"] === "coktan_secmeli" &&
      !empty($soru["secenekler"])
    ) {
      foreach ($soru["secenekler"] as $secenek) {
        $insSec->execute([$soruId, $secenek]);
      }
    }
  }
}

/** Admin anket listesi için soru sayısı haritası. */
function adminAnketSoruSayilari(PDO $db): array
{
  if (!dbTableExists($db, "anket_sorulari")) {
    return [];
  }
  try {
    $rows = dbFetchAll(
      $db,
      "SELECT anket_id, COUNT(*) AS c FROM anket_sorulari GROUP BY anket_id",
    );
    $map = [];
    foreach ($rows as $row) {
      $map[(int) $row["anket_id"]] = (int) $row["c"];
    }
    return $map;
  } catch (Throwable $e) {
    return [];
...
/**
 * Ankete katılan personelleri listeler.
 *
 * @return list<array{personel_id:int,ad:string,soyad:string,sicil_no:string,tamamlanma_tarihi:?string,cevap_sayisi:int}>
 */
function adminFetchAnketKatilimlari(PDO $db, int $anketId): array
{
  if ($anketId <= 0) {
    return [];
  }
  ...
  try {
    if (dbTableExists($db, "anket_katilimlari")) {
      return dbFetchAll(
        $db,
        "SELECT k.personel_id,
                p.ad,
                p.soyad,
                p.sicil_no,
                k.tamamlanma_tarihi,
                (
                  ...
                  WHERE c.anket_id = k.anket_id AND c.personel_id = k.personel_id
                ) AS cevap_sayisi
         FROM anket_katilimlari k
         INNER JOIN personeller p ON p.id = k.personel_id
         WHERE k.anket_id = ?
         ORDER BY k.tamamlanma_tarihi DESC, p.ad ASC, p.soyad ASC",
        [$anketId],
      );
    }
    ...
    return dbFetchAll(
      $db,
      "SELECT c.personel_id,
              p.ad,
              p.soyad,
              p.sicil_no,
              MIN(c.olusturma_tarihi) AS tamamlanma_tarihi,
              COUNT(*) AS cevap_sayisi
       FROM anket_cevaplari c
       INNER JOIN personeller p ON p.id = c.personel_id
       WHERE c.anket_id = ?
       GROUP BY c.personel_id, p.ad, p.soyad, p.sicil_no
       ORDER BY tamamlanma_tarihi DESC",
      [$anketId],
    );
  } catch (Throwable $e) {
    error_log("adminFetchAnketKatilimlari: " . $e->getMessage());
    return [];
  }
}

/**
 * Personelin anket katılımını ve cevaplarını siler; sayaç güncellenir.
 */
function adminAnketKatilimSil(PDO $db, int $anketId, int $personelId): bool
{
  ...
  try {
    ...
    if (dbTableExists($db, "anket_cevaplari")) {
      $db
        ->prepare(
          "DELETE FROM anket_cevaplari WHERE anket_id = ? AND personel_id = ?",
        )
    ...
    if (dbTableExists($db, "anket_katilimlari")) {
      $stmt = $db->prepare(
        "DELETE FROM anket_katilimlari WHERE anket_id = ? AND personel_id = ?",
      );
      $stmt->execute([$anketId, $personelId]);
      $silindi = $stmt->rowCount() > 0;
    }

    if (!$silindi && dbTableExists($db, "anket_cevaplari")) {
      // Yalnızca cevap vardı; yine de silindi kabul
      $silindi = true;
    }
    ...
    if (dbTableExists($db, "anket_katilimlari")) {
      ...
      $db
        ->prepare("UPDATE anketler SET katilim_sayisi = ? WHERE id = ?")
        ->execute([(int) ($row["c"] ?? 0), $anketId]);
    } else {
      $db
        ->prepare(
          "UPDATE anketler
           SET katilim_sayisi = GREATEST(0, katilim_sayisi - 1)
           WHERE id = ?",
        )
        ->execute([$anketId]);
    }
    ...
    error_log("adminAnketKatilimSil: " . $e->getMessage());
    return false;
  }
}
